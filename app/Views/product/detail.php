...
<?= $this->section('content') ?>
    ...
    <div class="relative w-full bg-white dark:bg-[#0b1120] min-h-screen">
        ...
        <div class="relative w-full h-[60vh] bg-slate-100 dark:bg-slate-900 group overflow-hidden z-0">
            ...
            <img src="<?= $imgSrc ?>" alt="<?= esc($p->name) ?>" class="w-full h-full object-cover transition-opacity duration-500 opacity-0 relative z-0" onload="document.getElementById('skel-hero').remove(); this.classList.remove('opacity-0');">
        ...
        <?= view_cell('App\Cells\ProductNav::render', ['p' => $p]) ?>
            ...
            <div class="px-6"> 
                <div class="mb-8 border-b border-slate-100 dark:border-slate-800 pb-8">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        <?php if(!empty($p->badges_array)): ?>
                            <?php foreach($p->badges_array as $b): ?>
                                <?php 
                                    // Adaptasi format baru (Array) vs lama (String)
                                    $label = is_array($b) ? $b['label'] : $b;
                                    $color = is_array($b) ? ($b['color'] ?? 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300') : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300';
                                    
                                    // Teks putih jika background berwarna
                                    $textColor = is_array($b) ? 'text-white' : '';
                                ?>
                                <span class="px-3 py-1 <?= $color ?> <?= $textColor ?> text-[10px] font-bold rounded-full uppercase tracking-wider shadow-sm border border-white/10">
                                    <?= esc($label) ?>
                                </span>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <div class="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
                        <div>
                            ...
                            <p class="text-2xl font-black font-mono text-slate-800 dark:text-slate-100 tracking-tight">
                ...
                <div class="mb-10">
                    ...
                    <div class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-medium whitespace-pre-wrap pl-4 border-l-2 border-slate-100 dark:border-slate-800"><?= esc($p->description) ?></div>
                </div>
                <?php endif; ?>
                    ...
                    <div class="space-y-4">
                        <?php if(empty($links)): ?>
                            <div class="py-12 text-center border-2 border-dashed border-emerald-500/30 rounded-3xl bg-emerald-50/50 dark:bg-emerald-900/10">
                                ...
                                <?php if(session()->get('isLoggedIn')): ?>
                                    <a href="<?= route_to('admin.link.add', (is_array($p)?$p['id']:$p->id)) ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95 font-bold text-xs tracking-widest uppercase animate-pulse"><span>➕ Tambah Link Pertama</span></a>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                            <?php foreach($links as $l): ?>
                                <?= view_cell('App\Cells\ShopOption::render', ['link' => $l, 'marketPrice' => $p->market_price, 'aiActive' => $aiActive ?? false]) ?>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                </div>
