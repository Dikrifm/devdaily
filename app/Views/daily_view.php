...
<body class="font-sans min-h-screen bg-slate-50 dark:bg-[#09090b] text-slate-800 dark:text-slate-200">
    ...
    <div class="relative h-72 w-full overflow-hidden group">
        ...
        <?php if($isAdmin): ?><a href="/index.php/admin/edit-product/<?= $p['id'] ?>" class="absolute top-6 right-6 w-10 h-10 glass rounded-full flex items-center justify-center text-white hover:bg-amber-500 hover:text-black transition z-20">✎</a><?php endif; ?>
    </div>
            ...
            <?php if(empty($links)): ?><div class="text-center py-10 opacity-50">Belum ada rekomendasi.</div><?php else: ?>
                ...
                <div class="glass rounded-2xl p-4 relative group hover:bg-white dark:hover:bg-slate-800 transition-colors">
                    ...
                    <div class="bg-slate-100 dark:bg-black/30 rounded-xl p-3 flex gap-3 items-start mt-3">
                        ...
                        <div class="flex-1">
                            <?php if($l['ai_comment']): ?><p class="text-xs text-slate-600 dark:text-slate-300 italic leading-relaxed">"<?= $l['ai_comment'] ?>"</p>
                            <?php else: ?><?php if($isAdmin): ?><a href="/index.php/admin/regenerate/<?= $l['id'] ?>" class="text-[10px] font-bold text-emerald-600 hover:underline">⚡ Cek Kata Ibu</a><?php endif; ?><?php endif; ?>
                        </div>
                    </div>
                    <?php endif; ?>
