...
<body class="font-sans min-h-screen transition-colors duration-500 bg-slate-100 text-slate-800 dark:bg-[#0f172a] dark:text-slate-200 relative overflow-x-hidden selection:bg-emerald-500 selection:text-white">
    ...
    <?php $isAdmin = session()->get('isLoggedIn'); // CEK STATUS LOGIN ?>
    ...
    <p class="text-[10px] font-bold opacity-60 tracking-[0.2em] uppercase mt-1">
        Kurasi Belanja Cerdas & Hemat
            ...
            <div class="flex gap-2">
                ...
                <?php else: ?>
                    <a href="/index.php/login" class="glass px-4 h-10 rounded-full flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest border-emerald-500/30">
                        LOGIN
                    </a>
                <?php endif; ?>
        ...
        <form action="/index.php" method="get" class="mb-6 relative z-10">
            <div class="relative group mb-3">
                ...
                <input type="text" name="q" value="<?= esc($keyword ?? '') ?>" placeholder="Cari data target..." class="relative w-full glass text-slate-900 dark:text-white py-4 pl-5 pr-12 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/50 font-semibold placeholder-slate-500">
        ...
        <div class="glass rounded-2xl p-6 mb-8 flex justify-between items-center">
            ...
            <div class="h-10 w-10 bg-emerald-500 rounded-xl flex items-center justify-center text-white shadow-lg">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
            </div>
        </div>

        <?php if(empty($products)): ?>
            <div class="text-center py-20 opacity-40 text-sm font-semibold">Data Kosong / Tidak Ditemukan</div>
        <?php else: ?>
    ...
    <script>
        ...
        function applyTheme() {
            if (localStorage.theme === 'dark' || (!('theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
                html.classList.add('dark'); iconSun.classList.remove('hidden'); iconMoon.classList.add('hidden');
            } else {
                html.classList.remove('dark'); iconSun.classList.add('hidden'); iconMoon.classList.remove('hidden');
            }
        }
